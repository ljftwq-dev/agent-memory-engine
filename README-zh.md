...
> **为什么？** 编程 agent 会"失忆"——每个新会话都从零开始，过去的决策、修复、约定全没了。这个引擎给它们廉价、持久的记忆：每个回合自动召回相关历史，每个完成的回合自动存入一条新记忆。
>
> 灵感来自上交大的 [MemRL 论文](https://arxiv.org/abs/2601.03192)，做了一个工程取舍：借鉴它的**两阶段检索 + 门控**，放弃完整 RL（对话没有干净的奖励信号——见 [design.md](docs/design.md)）。

> **重度自用（dogfooded），不是玩具 demo。** 作者的日常生产环境已积累 **3,300+ 条真实记忆**、**140+ 个 agent 会话**、连续 **4.5 个月**多 agent 实战。本设计的难点——近重复合并、"语义锚"短查询、会话隔离——全部来自这批真实负载的打磨，而非纸上谈兵。
